...
async def hi(event):
    await edit_or_reply(
        event,
        "⩹⌯⊷━♢ ⦓ S̴O̴U̴R̴C̴E̴ F̴L̴A̴S̴H̴ ⦔ ♢━⊶⌯⩺\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب فلاش حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر  وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب فلاش الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين\n⩹⌯⊷━♢ ⦓ S̴O̴U̴R̴C̴E̴ F̴L̴A̴S̴H̴ ⦔ ♢━⊶⌯⩺\n**⌯︙ [┈┉━｢ 𝑠𝑜𝑢𝑟𝑐𝑒 𝑓𝑙𝑎𝑠ℎ ｣━┅┈](t.me/FLS_44)**",
        link_preview=False,
    )
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "**• يبدو انك لم تقوم بأضافة مستخدم متحكم فلاش الخاص بك**"
        )
    result = "**قائمة المستخدمين المتحكمين بتنصيب فلاش :**\n\n"
