...
from . import edit_delete, WWWL5, logging
...
autopic_path = os.path.join(os.getcwd(), "WWWL5", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "WWWL5", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "WWWL5", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await WWWL5.upload_file(autophoto_path)
        try:
            if i > 0:
                await WWWL5(
                    functions.photos.DeletePhotosRequest(
                        await WWWL5.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await WWWL5(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await WWWL5(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await WWWL5(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@WWWL5.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
WWWL5.loop.create_task(digitalpicloop())
WWWL5.loop.create_task(autoname_loop())
WWWL5.loop.create_task(autobio_loop())
