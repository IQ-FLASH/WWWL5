...
@WWWL5.on(admin_cmd(pattern=r"ضيف ?(.*)"))
async def get_users(event):   
    ...
    WWWL5 = await get_chatinfo(event) ; chat = await event.get_chat()
    if event.is_private:
              return await roz.edit("**▾∮ لا يمكننـي اضافـة المـستخدمين هـنا**")    
    s = 0 ; f = 0 ; error = 'None'   
  
    await roz.edit("**▾∮ حـالة الأضافة:**\n\n**▾∮ تتـم جـمع معـلومات الـمستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(WWWL5.full_chat.id):
                try:
...
@WWWL5.on(admin_cmd(pattern=r"اضافة_جهاتي ?(.*)"))
