...
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس فلاش " + "**• حدث خطأ ما :**\n"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس فلاش"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس فلاش أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/SOURCE-FLASH/TELEHON"
    os.chdir("/app")
    try:
