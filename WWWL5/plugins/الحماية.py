...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"-- - حُذَرَتْڴ وَقًوَلُتْلُڴ مٌتْبّْعتْشِ ڴتْيـرَ تْمٌ حُظٌرَڴ بّنَجْاحُ مٌقًدِرَشِ اٌخلُيـڴ تْوَجْْع دِمٌاغً الُمٌالُڴ \n- - يـلُا بّاي 🌚🤍"
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""- هلا حياتي {mention}  ،
    . ᯏ𝖲᥆υᖇᥴᥱ - 𝖥𝗅ᥲ᥉𝗁 ᭡ .
✦┅━╍━╍╍━━╍━━╍━┅✦
~ يـمٌڴنَ اڴـوَنَ نَـايـمٌ اوَ مٌشِغًوَلُ .   ⚡️
~ سِيـبّ رَسِـالُـتْـڴ وَانَـتْـظٌـرَ الُـرَدِ .   ⚡️
~ ٌخلُيـڴ ٌخفَيـفَ وَمٌتْبّْعتْشِ ڴتْيـرَ.   ⚡️
~ ْهيـڴوَنَ عنَدِڴ {warns}/{totalwarns} رَسِائلُ فَقًطِ  .  ⚡️
~ وَرَإهمٌ تْتْحُظٌرَ اذَا فَضُلُتْ تْبّْعتْ .  ⚡️
━═━═━═━═━═━═━═━"""
    else:
        USER_BOT_NO_WARN = f"""- هلا حياتي {mention}  ،
    . ᯏ𝖲᥆υᖇᥴᥱ - 𝖥𝗅ᥲ᥉𝗁 ᭡ .
...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5627420357:
        await event.client.send_message(chat, "⪼ اهـلا مـبرمج الـسورس الغـالي اننـي محظـوظ لقدومـك الـي 🙈♥️𓆰")
        reason = "❃ **يجب عليك الترحيب بمطور السورس**"
    if event.chat_id == 6456641797:
        await event.client.send_message(chat, "⪼ اهـلاً بك مـبرمج الـسورس ، تمت الموافقه تلقائياً على ارسالك الرسائل ♥️𓆰")
        reason = "❃ **يجب عليك الترحيب بمطور السورس**"
        try:
            PM_WARNS = sql.get_collection("pmwarns").json
        except AttributeError:
...
async def approve_p_m(event):
    ...
    if not pmpermit_sql.is_approved(user.id):
        ...
        await edit_delete(
            event,
            f"-  [{user.first_name}](tg://user?id={user.id})\n❃╎تـم قبوله لإرسـال الـرسـائـل ⚠️ \n❃╎ الـسـبـب ❔ : {reason}",
        )
        try:
            PMMESSAGE_CACHE = sql.get_collection("pmmessagecache").json
        except AttributeError:
...
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
        return await edit_or_reply(
            event,
            f"__يجب عليك تفعيل امر__`{cmdhd}تشغيل الحماية` __ليمكنك استخدام هذه الميزه__",
        )
    try:
        PM_WARNS = sql.get_collection("pmwarns").json
    except AttributeError:
