...
@WWWL5.iq_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@WWWL5.iq_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
async def on_plug_in_callback_query_handler(event):
    ...
    sqllist.rm_from_list("pmoptions", event.query.user_id)
    await event.edit(text)
@WWWL5.on(admin_cmd(pattern="الحماية (تشغيل|ايقاف)(?: |$)(.*)"))
...
async def pmpermit_on(event):
    ...
    if input_str == "ايقاف":
        ...
        await edit_delete(event, "**⁂ ⦙  امـر الـحـمـايـة بـالـفـعـل مُـمَـكـن لـحـسـابـك  🌿**")
@WWWL5.on(admin_cmd(pattern="(ق|قبول)(?:\s|$)([\s\S]*)"))
...
@WWWL5.on(admin_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)"))
...
@WWWL5.on(admin_cmd(pattern="مرفوض(?:\s|$)([\s\S]*)"))
...
@WWWL5.on(admin_cmd(pattern="مقبول(?:\s|$)([\s\S]*)"))
...
@WWWL5.on(admin_cmd(pattern="المقبولين(?: |$)(.*)"))
