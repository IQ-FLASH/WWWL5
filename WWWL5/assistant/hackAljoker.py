...
'''
mm = '''
قم بلأنضمام الى قناة فلاش @FLS_44
'''
...
keyboard = [
  ...
  [
    Button.url("سورس فلاش 🕷", "https://t.me/FLS_44")
    ]
]
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/P_O28")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس فلاش", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك فلاش.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك فلاش.", buttons=keyboard)
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/FLS_44")
        ]
    ]
